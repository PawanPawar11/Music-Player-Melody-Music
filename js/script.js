...
function prevMusic(){
    ...
    musicIndex--;
    //if musicIndex is less than 1 then musicIndexwill be array length so the last song will play
...
//let's update playing song current time accourding to the progress bar width
...
//let's work on repeat, shuffle song accourding ot the icon
const repeatBtn = wrapper.querySelector("#repeat-plist");
repeatBtn.addEventListener("click", ()=>{
    //first we get the inner Text of the icon then we'll change accourdingly
...
mainAudio.addEventListener("ended", ()=>{
    //we'll do accourding to the icon means if user has set icon to loop song then we'll repeat
    ...
    switch(getText){
        ...
        nextMusic();
        break;
        case "repeat_one": //if this icon is repeat_one then we'll change the current playing song current time to 0 so song will play from the beginning
        ...
        musicIndex = randIndex; //passing randomIndex of musicIndex so the random song will play
...
function playingNow(){
    ...
    //let's remove playing class from all other li expect the last one which is 
